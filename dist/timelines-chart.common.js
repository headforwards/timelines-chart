...
var TimeOverview = Kapsule({
  ...
  init: function init(el, state) {
    ...
    state.brush = d3Brush.brushX().on('end', function () {
      ...
      var selection = d3Selection.event.selection ? d3Selection.event.selection.map(state.timeScale.invert) : state.timeScale.domain();
  ...
  update: function update(state) {
    ...
    state.timeScale = (state.useUtc ? d3Scale.scaleUtc : d3Scale.scaleTime)().domain(state.domainRange).range([0, brushWidth]);
...
var timelines = Kapsule({
  props: {
    ...
    zQualitative: { default: false, onChange: function onChange(discrete, state) {
        state.zColorScale = discrete ? d3Scale.scaleOrdinal([].concat(toConsumableArray(d3Scale.schemeCategory10), toConsumableArray(d3Scale.schemeCategory20b))) : d3Scale.scaleSequential(d3ScaleChromatic.interpolateRdYlBu); // alt: d3.interpolateInferno
      }
    },
