...
import { scaleLinear, scaleOrdinal, scalePoint, scaleSequential, scaleTime, scaleUtc, schemeCategory10, schemeCategory20b } from 'd3-scale';
...
var TimeOverview = Kapsule({
  ...
  update: function update(state) {
    ...
    state.timeScale = (state.useUtc ? scaleUtc : scaleTime)().domain(state.domainRange).range([0, brushWidth]);
...
var timelines = Kapsule({
  props: {
    ...
    zQualitative: { default: false, onChange: function onChange(discrete, state) {
        state.zColorScale = discrete ? scaleOrdinal([].concat(toConsumableArray(schemeCategory10), toConsumableArray(schemeCategory20b))) : scaleSequential(interpolateRdYlBu); // alt: d3.interpolateInferno
      }
    },
