// Version 2.5.0 timelines-chart - https://github.com/vasturiano/timelines-chart
...
var d3 = Object.freeze({
	...
	scaleUtc: d3ScaleUtc,
	schemeCategory10: d3SchemeCategory10,
	schemeCategory20b: d3SchemeCategory20b,
...
var TimeOverview = Kapsule({
  ...
  init: function init(el, state) {
    ...
    state.brush = brushX().on('end', function () {
      ...
      var selection = event.selection ? event.selection.map(state.timeScale.invert) : state.timeScale.domain();
      state.onChange.apply(state, toConsumableArray$1(selection));
    });
  ...
  update: function update(state) {
    ...
    state.timeScale = (state.useUtc ? d3ScaleUtc : d3ScaleTime)().domain(state.domainRange).range([0, brushWidth]);

    state.xAxis.scale(state.timeScale).tickFormat(state.tickFormat);
    state.xGrid.scale(state.timeScale).tickSize(-brushHeight);
    ...
    state.svg.select('.x.grid').attr('transform', 'translate(0,' + brushHeight + ')').call(state.xGrid).selectAll('.tick').classed("minor", function (d) {
      return d.getHours();
    });

    state.svg.select('.x.axis').attr("transform", "translate(0," + brushHeight + ")").call(state.xAxis).selectAll('text').attr('y', 8);

    state.svg.select('.brush').call(state.brush.extent([[0, 0], [brushWidth, brushHeight]])).call(state.brush.move, state.currentSelection.map(state.timeScale)).selectAll('rect').attr('height', brushHeight);
  }
});
...
var timelines = Kapsule({
  props: {
    ...
    zQualitative: { default: false, onChange: function onChange(discrete, state) {
        state.zColorScale = discrete ? ordinal([].concat(toConsumableArray$1(d3SchemeCategory10), toConsumableArray$1(d3SchemeCategory20b))) : sequential(interpolateRdYlBu); // alt: d3.interpolateInferno
      }
    },
  ...
  update: function update(state) {
    ...
    function setupDimensions() {
      ...
      if (state.overviewArea) {
        state.overviewArea.useUtc(state.useUtc).width(state.width * 0.8).height(state.overviewHeight + state.overviewArea.margins().top + state.overviewArea.margins().bottom);
      }
    }
    ...
    function renderAxises() {
      ...
      // X
      state.xAxis.scale(state.xScale).ticks(Math.round(state.graphW * 0.0011));
